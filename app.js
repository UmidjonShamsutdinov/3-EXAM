...
const bannerContent = document.querySelector(".hero__content")
...
function heroRender(data){
    ...
    data.results.forEach(e => {
        ...
        div.appendChild(img)
        div.appendChild(heroContent)
        fragment.appendChild(div)
    })

    mainSwiper.appendChild(fragment)
}
...
function innerRender(date){
    ...
    date.results.forEach(e => {
        ...
        div.innerHTML = `
        <a href="${"./movie_info/index.html?infoId="+elementId}"><img src="${imgLink + e.poster_path}" alt="image"/></a>
        <p class="title" title="${e.name}">${e.name.split("").length>10 ? e.name.slice(0, 11) + "..." : e.name}</p>
        `
...
function otherLinesRender(date){
    ...
    date.results.forEach(e => {
        ...
        const imgLink = "https://image.tmdb.org/t/p/original/"
        div.innerHTML = `
        <a href="${"./movie_info/index.html?infoId="+elementId}"><img src="${imgLink + e.backdrop_path}" alt="image"/></a>
